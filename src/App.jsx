...
export default function MobilePreview() {
  ...
  async function fetchNrbRate(currencyIso3 = "USD") {
    ...
    try {
      ...
      const res = await fetch(`https://www.nrb.org.np/api/forex/v1/rates?page=1&per_page=1&from=${today}&to=${today}`);
      const data = await res.json();
      const day = data?.data?.payload?.[0];
      const currencyRate = day?.rates?.find((r) => r.currency?.iso3 === currencyIso3);
      if (!currencyRate) throw new Error(`${currencyIso3} not found in NRB response`);
      const sellRate = parseFloat(currencyRate.sell);
      setUsdRateInfo({ status: "success", rate: sellRate, date: day.date, currency: currencyIso3 });
      setForm((f) => ({ ...f, usd_rate: sellRate }));
    } catch (e) {
      // Likely a CORS restriction from NRB's server on browser-based requests,
      // or no rate published yet for today (weekends/holidays) — either way,
      // fall back to manual entry rather than blocking the quote.
